...
class Trainer:
    ...
    def training_step(self, data_loader):
        ...
        for batch in tqdm(data_loader):
            ...
            self._optimizer.zero_grad()  # Zero out gradients for next batch
            train_loss += loss.item() * len(batch.input_ids)  # Update train_loss
            n_tks += len(batch.input_ids)  # Update the total number of tokens
    ...
    def get_loss(self, logits, lbs):
        ...
        # --- TODO: start of your code ---
        # Create a mask to filter out MASKED_LB_ID in lbs
        active_loss = lbs.view(-1) != MASKED_LB_ID
        active_logits = logits.view(-1, logits.shape[-1])[active_loss]
        active_lbs = lbs.view(-1)[active_loss]

        loss = self._loss(active_logits, active_lbs)
        return loss
        # --- TODO: end of your code ---
    ...
    def evaluate(self, dataset, detailed=False):
        ...
        # --- TODO: start of your code ---
        pred_lbs = []
        label_to_idx = {'O': 0, 'B-PER': 1, 'I-PER': 2, 'B-LOC': 3, 'I-LOC': 4, 'B-ORG': 5, 'I-ORG': 6, 'B-MISC': 7, 'I-MISC': 8}
        idx_to_label = {value: key for key, value in label_to_idx.items()}

        # Predict labels for each batch and convert to string labels
        for batch in data_loader:
            ...
            # Convert the label indices in the ground truth to -100 where padding
            true_label_indices = batch.labels.cpu().numpy()
            mask = true_label_indices != MASKED_LB_ID

            # For each sequence in the batch, consider only the labels where ground truth isn't MASKED_LB_ID
            for idx_sequence, valid_labels in zip(predicted_indices, mask):
                valid_indices = [idx for idx, is_valid in zip(idx_sequence, valid_labels) if is_valid]
                pred_labels = [idx_to_label[idx] for idx in valid_indices]
